...
function mbvm_load_teachers(bool $activeOnly = false): array
{
    ...
    foreach ($teachers as &$t) {
        if (isset($t['photo_path']) && strpos((string) $t['photo_path'], '/') === 0) {
            $t['photo_path'] = ltrim((string) $t['photo_path'], '/');
        }
    }
    unset($t);
...
function mbvm_env_val(string $key): string
{
    $val = getenv($key);
    if ($val !== false && $val !== '') {
        return (string) $val;
...
{
    return mbvm_env_val('MBVM_ADMIN_USERNAME') ?: 'admin';
}

function mbvm_admin_password(): string
{
    return mbvm_env_val('MBVM_ADMIN_PASSWORD') ?: 'admin123';
}

function mbvm_admin_password_hash(): string
{
    return mbvm_env_val('MBVM_ADMIN_PASSWORD_HASH');
}
...
function mbvm_require_admin(): void
{
    if (empty($_SESSION['mbvm_admin'])) {
        $target = file_exists('login.html') ? 'login.html' : '../login.html';
