...
$checks = [
    ...
    'admin_password' => [
        'label' => 'Admin authentication configured',
        'ok' => mbvm_admin_password_hash() !== '' || mbvm_admin_password() !== '',
        'detail' => mbvm_admin_password_hash() !== '' ? 'Custom Hash' : 'Default Credentials',
    ],
];
...
if ($wantsJson) {
    ...
    echo json_encode([
        'success' => $allOk,
        'healthy' => $allOk,
        'message' => $allOk ? 'Application and database are operational.' : 'Backend health check reported issues.',
        'checks' => $checks
