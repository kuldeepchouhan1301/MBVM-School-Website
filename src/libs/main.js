...
function is_visible(el) {
  ...
  if (!offset || typeof offset.top === 'undefined') return false;
  var dif = offset.top - $(window).scrollTop();
...
function sticky() {
  ...
  if ($('.sticky-wrapper').length) {
    ...
    var orgElementTop = (orgElementPos && typeof orgElementPos.top !== 'undefined') ? orgElementPos.top : 0;
...
function add_button_menu() {
  if ($('#navToggle').length || $('.mobile_menu_switcher').length) return;
...
function double_menu() {
  ...
  var heightElement = offset.top + element.innerHeight();
...
$(document).ready(function () {
  ...
  $(document).on("mouseenter", ".event-calendar td, .week-view-wrap td", function () {
    ...
    if (!thisOffset || !parentOffset) return;

    var topChild = thisOffset.top;
    var HeightChild = $(this).height();
    var BottomChild = topChild + HeightChild;

    var topParent = parentOffset.top;
...
jQuery(window).load(function () {
  ...
  $('.scroll-down').on('click', function () {
    var target = $('#home').length ? $('#home') : $('body');
    var offset = target.offset();
    var top = offset ? offset.top : 0;
    $('html, body').animate({ scrollTop: top }, { duration: 1500, easing: "easeInOutExpo" });
    return false;
  });
...
function mobile_menu_controller_init() {
  return; // Disabled: modern MBVM mobile menu is handled by #navToggle and #mainNav
  window.mobile_nav = {
...
/* \mobile menu */
/* Disable legacy jQuery Validate intercept on .contact-form to allow contact-actions.js to handle modern AJAX submission */
...
/* Centralized Modern MBVM Mobile Navigation Handler */
$(document).ready(function () {
  $(document).on('click', '#navToggle', function (e) {
    ...
    var isOpen = nav.hasClass('is-active');
  ...
  $(document).on('click', '#mainNav a', function () {
    if ($(window).width() < 1024) {
      ...
      $('#navToggle').attr('aria-expanded', 'false').find('i').removeClass('fa-times').addClass('fa-bars');
    }
  });
